...
class SolrPaginator:
    ...
    def page(self, page_num=1):
        ...
        if page_num not in self.page_range:
            raise Exception('EmptyPage,That page does not exist.')

        # Page 1 starts at 0; take one off before calculating
        start = (page_num - 1) * self.page_size
        new_result = self._fetch_page(start=start)
        return SolrPage(new_result.results, page_num, self,response=new_result)


class SolrPage:
    """A single Paginator-style page."""

    def __init__(self, result, page_num, paginator,response=None):
        self.result = result
